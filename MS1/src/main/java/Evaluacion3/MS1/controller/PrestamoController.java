...
public class PrestamoController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Éxito", content = @Content(mediaType = "application/hal+json", schema = @Schema(implementation = PrestamoDTO.class))),
    ...
    public ResponseEntity<CollectionModel<EntityModel<PrestamoDTO>>> obtenerTodosPrestamos() {
        ...
        if (prestamos.isEmpty()) return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        return ResponseEntity.ok(CollectionModel.of(prestamos, linkTo(methodOn(PrestamoController.class).obtenerTodosPrestamos()).withSelfRel()));
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "Creado", content = @Content(mediaType = "application/hal+json", schema = @Schema(implementation = PrestamoDTO.class))),
    ...
    public ResponseEntity<EntityModel<PrestamoDTO>> agregarPrestamo(@Valid @RequestBody Prestamo prestamo) {
        try {
            ...
            return ResponseEntity.created(linkTo(methodOn(PrestamoController.class).buscarPorId(dtoCreado.getIdPrestamo())).toUri()).body(assembler.toModel(dtoCreado));
        } catch (Exception e) {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Actualizado", content = @Content(mediaType = "application/hal+json", schema = @Schema(implementation = PrestamoDTO.class))),
        @ApiResponse(responseCode = "404", description = "No encontrado", content = @Content)
    })
